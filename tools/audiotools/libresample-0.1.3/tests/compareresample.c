...
void dostat(char *name, double *d1, double *d2, int len)
{
   int i;
...
void runtest(double *src, int srclen,
             double *ans, int anslen,
             double factor)
{
   struct timeval tv0, tv1;
   int dstlen = (int)(srclen * factor);
   double *dst_rs = (double *)malloc((dstlen+100) * sizeof(double));
   double *dst_rabbit = (double *)malloc((dstlen+100) * sizeof(double));
   void *handle;
   SRC_DATA rabbit;
   double deltat;
...
int main(int argc, char **argv)
{
   ...
   printf("\n*** sin wave, factor = 1.0 *** \n\n");
   srclen = 100000;
   src = malloc(srclen * sizeof(double));
   for(i=0; i<srclen; i++)
      src[i] = sin(i/100.0);
   ...
   for(i=0; i<srclen; i++)
      src[i] = sin(i/100.0);
   ans = malloc((srclen/4) * sizeof(double));
