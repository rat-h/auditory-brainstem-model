...
int main(int argc, char **argv)
{
   ...
   srci = (double *)malloc(srclen * channels * sizeof(double));
   dsti = (double *)malloc(dstlen * channels * sizeof(double));
   src = (double *)malloc(srclen * sizeof(double));
   dst = (double *)malloc(dstlen * sizeof(double));
   ...
   while(pos < srcinfo.frames) {
      ...
      sf_readf_double(srcfile, &srci[bufferpos*channels], block);
      block += bufferpos;
         ...
         for(i=0; i<out; i++)
	 {
      ...
      sf_writef_double(dstfile, dsti, out);
